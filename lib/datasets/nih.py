...
class nih(imdb):
    def __init__(self, image_set, devkit_path=None):
        ...
        self._roidb_handler = self.gt_roidb #{'boxes''gt_classes''gt_overlaps''flipped''seg_areas'} for each box in annotations
        ...
        assert os.path.exists(self._devkit_path), \
            'NIHdevkit path does not exist: {}'.format(self._devkit_path)
    ...
    #     format.
    #     """
    #     filename = os.path.join(self._data_path, 'Annotations', 'BBox_List_2017' + '.xml')
    ...
    def _write_nih_results_file(self, all_boxes):
        for cls_ind, cls in enumerate(self.classes):
            if cls == '__background__':
                continue
            print('Writing {} NIH results file'.format(cls))
            filename = self._get_nih_results_file_template().format(cls)
    ...
    def _do_python_eval(self, output_dir='output'):
        ...
        annopath = os.path.join(
            self._data_path,
            'Annotations', 'BBox_List_2017' + '.csv')
